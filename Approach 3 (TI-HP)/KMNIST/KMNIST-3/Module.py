...
mn = MNISTUitl()
X, Y, x, y = mn.getdata2(0,0,sx,sy)
#nm , xt, yt = mn.trainDense3(X, Y, x,y,sx,sy,10,20)
#nm.save('mnist.h5')
...
for j in labs:
    ...
    mc = 0
    for x in digit:
        W1, W2, W3, W4, b1, b2, b3, b4= slc.dynamicmodify(slicingModel,x,sx,sy)
        mc=mc+1
    ...
    mc = 0
    for x in digit:
        W1, W2, W3, W4, b1, b2, b3, b4= slc.modifyThroughInterSection(slicingModel,x,sx,sy)
        mc=mc+1
        if np.count_nonzero(slc.D4) < 45:
            #print("Breaking at mc ", mc,np.count_nonzero(slc.D2))
            slc.first = True
...
# nm=load_model('kmnist.h5')
# acct=nm.predict(xt).argmax(axis=-1)
# acct=np.mean(np.equal(acct, yt))
# print("Model Test Accuracy:",str(acct))
    #In[]:
