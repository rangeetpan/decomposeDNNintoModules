...
class MNISTUitl:
    ...
    # the data, split between train and test sets
        ...
        x_zo = []
        y_zo = []
        x_train=x_train1[y_train1!=5]
        temp=x_train1[y_train1==5]
        x_train=np.append(x_train,temp[0:500], axis=0)
        y_train=[]
        for x in y_train1[y_train1!=5]:
            y_train.append(x)
        y_train=np.array(y_train)
        temp=[]
        for x in y_train1[y_train1==5][0:500]:
            temp.append(5)
        ...
        xt_zo = []
        yt_zo = []
        x_test=x_test1[y_test1!=5]
        temp=x_test1[y_test1==5]
        x_test=np.append(x_test,temp[0:50], axis=0)
        y_test=[]
        for x in y_test1[y_test1!=5]:
            y_test.append(x)
        y_test=np.array(y_test)
        temp=[]
        for x in y_test1[y_test1==5][0:50]:
            temp.append(5)
