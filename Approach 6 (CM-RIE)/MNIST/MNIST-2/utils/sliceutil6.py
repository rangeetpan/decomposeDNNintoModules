...
class Slice:
    ...
    def getweights(self, nm):
        ...
        w6,b6 = nm.layers[6].get_weights()
        w7,b7 = nm.layers[7].get_weights()
        W1 = np.vstack([w1])
        W2 = np.vstack([w2])
        W3 = np.vstack([w3])
        W4 = np.vstack([w4])
    ...
    def dynamicmodify(self, nm, x,img_rows = 28, img_cols = 28):
        ...
        X6 = np.dot(X5,self.W6)
        X6 = np.add(X6,self.b6)
        X7 = np.dot(X6,self.W7)
        X7 = np.add(X7,self.b7)
        X7 = self.softmax(X7)
        X1[X1<0]=0
        ...
        for i in range(X6.shape[0]):
            if X6[i] <= 0:
                #print("Zero X1")
                self.D6[:,i] = [x for x in self.D6[:,i]]
            else:
                self.D6[:,i] = self.W6[:,i]
                self.d6[i] = self.b6[i]
        for i in range(X7.shape[0]):
            if X7[i] <=.5:
                #print("Zero X2")
                self.D7[:,i] = [x for x in self.D7[:,i]]
            else:
                self.D7[:,i] = self.W7[:,i]
                self.d7[i] = self.b7[i]
        ...
        #print("D2")
        #print(self.D2)
        return self.D1, self.D2, self.D3, self.D4, self.D5, self.D6, self.D7, self.d1, self.d2, self.d3, self.d4, self.d5, self.d6, self.d7
    ...
    def modifyThroughInterSection(self,nm, x,img_rows = 28, img_cols = 28):
        ...
        for i in range(X5.shape[0]):
            if X5[i] <= 0:
                #print("Zero X1")
                self.D5[:,i] = [0 for x in self.D5[:,i]]
                self.D6[i,:]=[0 for x in self.D6[i,:]]
                self.d5[i] = 0
            else:
                if self.first == True:
                    self.D5[:,i] = self.W5[:,i]
        ...
        for i in range(X6.shape[0]):
            if X6[i] <= 0:
                #print("Zero X1")
                self.D6[:,i] = [0 for x in self.D6[:,i]]
                self.D7[i,:]=[0 for x in self.D5[i,:]]
                self.d6[i] = 0
            else:
                if self.first == True:
                    self.D6[:,i] = self.W6[:,i]
                    ...
                    for j in range(0,len(self.W6[:,i])):
                        if self.W6[j,i] < 0 :
                            self.D6[j,i] = max(self.D6[j,i], self.W6[j,i])
                            if(self.D6[j,i]<0):
                                self.D6[j,i]=0
                        else:
                            self.D6[j,i] = min(self.D6[j,i], self.W6[j,i])
                    self.d6[i] = self.b6[i]
        for i in range(X7.shape[0]):
            if X7[i] <= .0001:
                #print("Zero X2")
                self.D7[:,i] = [ 0 for x in self.D7[:,i]]
                self.b7[i] = 0
            else:
                if self.first == True:
                    self.D7[:,i] = self.W7[:,i]
                    ...
                    for j in range(0,len(self.W7[:,i])):
                        if self.W7[j,i] < 0 :
                            self.D7[j,i] = max(self.D7[j,i], self.W7[j,i])
                            # if(self.D2[j,i]<0):
                            #     self.D2[j,i]=0
                        else:
                            self.D7[j,i] = min(self.D7[j,i], self.W7[j,i])
                        #self.D2[:,i] = self.W2[:,i]
                    self.d7[i] = self.b7[i]
