...
mn = MNISTUitl()
X, Y, x, y = mn.getdata2(0,0,sx,sy)
#nm , xt, yt = mn.trainDense3(X, Y, x,y,sx,sy,10,50)
#nm.save('fmnist.h5')
...
for j in labs:
    ...
    #test=[]
    slicingModel=load_model('fmnist.h5')
    #X, Y, x, y = mn.getdata2(0,0,sx,sy)
    model=load_model('fmnist.h5')
    digit  = []
    slc = Slice()
    W1, W2, W3, W4, W5, b1, b2, b3, b4, b5 = slc.getweights(slicingModel)
    ...
    mc = 0
    for x in digit:
        W1, W2, W3, W4, W5, b1, b2, b3, b4, b5 = slc.modifyThroughInterSection(slicingModel,x,sx,sy)
        mc=mc+1
        if np.count_nonzero(slc.D5) < 45:
    ...
    mc = 0
    for x in digit:
        W1, W2, W3, W4, W5, b1, b2, b3, b4, b5 = slc.dynamicmodify(slicingModel,x,sx,sy)
        mc=mc+1
        if np.count_nonzero(slc.D5) < 45:
            #print("Breaking at mc ", mc,np.count_nonzero(slc.D2))
            slc.first = True
    ...
    model.layers[4].set_weights([slc.D4,slc.d4])
    model.layers[5].set_weights([slc.D5,slc.d5])
